...
def schemeRuntime : String :=
  ...
  "\n".intercalate
    [ ";; Malgo Runtime Support",
      ...
      ";; jq/gh --json consumers. open-process-ports already runs its",
      ";; argument through one /bin/sh -c layer, so the quoted command line",
      ";; needs no further shell wrapping.",
      ...
      "               [code (if marker-idx",
      "                         (string->number (substring err-full (+ marker-idx 12) (string-length err-full)))",
      "                         0)])",
