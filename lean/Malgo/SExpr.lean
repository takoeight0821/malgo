...
Caveat: Haskell renders `Float`/`Double` via `show` (shortest round-trip
decimal), which `Float.toString` does not reproduce; the three
float-bearing golden groups are regenerated for Lean per the plan. -/
def render : Atom → String
  | .symbol t => t
  | .int n none => toString n
  | .int n (some t) => toString n ++ "_" ++ t
  | .float n => toString n ++ "_f32"
  | .double n => toString n ++ "_f64"
