...
unifyInternal pos (TMu body) t2 = do
  ...
  case mode of
    ...
    IsoRecursive ->
      throwError $ UnificationError pos (TMu body) t2 "iso-recursive mismatch: TMu on left requires explicit unfold"
...
unifyInternal pos t1 (TMu body) = do
  ...
  case mode of
    ...
    IsoRecursive ->
      throwError $ UnificationError pos t1 (TMu body) "iso-recursive mismatch: TMu on right requires explicit fold"
-- Arrow types
