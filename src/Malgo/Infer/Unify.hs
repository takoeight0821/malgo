...
unifyInternal pos (TMu body) t2 = do
  ...
  case mode of
    ...
    IsoRecursive ->
      throwError $ UnificationError pos (TMu body) t2 "iso-recursive mismatch: cannot unify TMu with non-recursive type without explicit unfold"
...
unifyInternal pos t1 (TMu body) = do
  ...
  case mode of
    ...
    IsoRecursive ->
      throwError $ UnificationError pos t1 (TMu body) "iso-recursive mismatch: cannot unify non-recursive type with TMu without explicit fold"
-- Arrow types
