...
Go バックエンド（#482）は当初 Level 1 で 0.41s、Zig の 0.24s に対して 1.7 倍遅かった。その内訳を実測で特定し、改善案が本当に効くかを 1 件ずつ検証した記録。

**結論から**:

- **採用したのは 4 件**（join point のインライン化、固定引数、文字列のスカラキャッシュ、文字列カーソル）。Level 1 は **0.41s → 0.27s**、`BenchFibDeep` は 0.48s → 0.29s となり、fib-deep では Zig（0.31s）を上回った。
- **見送り・却下が 9 件。** interface boxing の除去、`[]rune` キャッシュ（2 倍遅い）、トランポリンの形の変更、`mkStruct` の可変長引数除去、整数タグ、generics、リフレクション、escape 判定のレキシカル化、scrutinee タプル融合。
- **割り当て回数はボトルネックではない**（§4.12）。独立した 3 つの実験が一致した——malloc を 35%、確保バイト数を 32% 減らしても時間は 0.3% も動かない。fib-deep では Go の escape 解析が既にスタック化していた。
- **Chez との比較は、コーパス全体で Go が 16.3 倍速い**（golden 86 ケースの合計で 0.76s 対 12.39s）。Chez は 1 ケースごとにスクリプトをコンパイルし直すため、その分が実行時間に乗る。
- **生成コードの実行速度そのものでは Chez が 3 倍前後速い。** Chez が仕事を減らしているのではない——Level 1 で Chez は Go の 1.8 倍割り当てている。割り当て・dispatch・実処理の 3 つすべてで単価が 3〜4 倍違い、単一のレバーでは埋まらない。
...
### 4.7 `Struct.Tag` を文字列から整数へ — 効果なし

Go 固有の期待として、`Struct{Tag string; Fields []Value}`（40 バイト、サイズクラス 48、ポインタ 2 本）を `Tag uint32` にすれば 32 バイトのサイズクラスに落ち、GC が走査するポインタも 1 本に減る。タグ比較も文字列比較から整数比較になる。

生成ソースの 129 個のタグに索引を振って書き換え、`tagNames` テーブルを添えて実測した。

**0.30s → 0.30s。差が出なかった。**

Go の文字列比較は、同一バイナリ内の定数文字列どうしならポインタ一致で短絡する。サイズクラスの変化も、割り当てが全体の約 25% で `Struct` はそのうちの一部という構成では表に出ない。

### 4.8 `args []Value` を固定引数へ — 有効、3〜6%

`Fn func(args []Value) Action` を `Fn func(a, b Value) Action` にし、`Action` から `Argc` を落とす。dispatch ごとの slice header 構築と、引数読み出しごとの境界チェックが消える。

| | 変更前 | 変更後 |
|---|---|---|
| Level 1 | 0.30s | **0.29s** |
| fib-deep | 0.32s | **0.30s** |

微小ベンチでは dispatch が 4.84 → 3.63 ns（-25%）。実プログラムでは dispatch 以外の仕事があるぶん薄まって 3〜6% になる。

副次的に、引数が値渡しになったので `run` のループが `cur = cur.Code(cur.A, cur.B)` と直接代入できるようになった。以前は呼び出し先が `cur.Argv` を読んでいる最中に `cur` を上書きしうるため、別スロットを経由していた。

### 4.9 `GOGC` の調整 — 採用しない

live heap が 1〜6MB と小さく、全体が短命なので、GC の回数を減らせば効くはずだった。

| | 既定 (100) | 400 | 800 | off |
|---|---|---|---|---|
| Level 1（固定引数前） | 0.30s | 0.28s | 0.29s | 0.29s |
| fib-deep（固定引数前） | 0.33s | 0.31s | 0.33s | — |

6% ほど出ていたが、**4.8 の固定引数化を入れた後は差が消えた**（Level 1 0.29 → 0.29、fib-deep 0.30 → 0.31）。GC はもともと時間の 1〜2% であり、見えていた差は割り当て速度側の変化に吸収された。生成バイナリの GC ポリシーを既定から動かす理由は無い。

`GOMAXPROCS=1` は 0.30 → 0.40s と悪化する。単一 goroutine でも背景マークが並行に走るほうが速い。

### 4.10 レキシカルクロージャ向けに escape 判定を緩める — 効果なし

`classifyJoins` は「入れ子の `lambda`/`object`/`mu` の自由変数は escape」とし、さらに `promoteEscapingCaptures` で閉包を取る。どちらも **Zig が escaping join を「captures 配列を持つトップレベル関数」へリフトする**ために必要な規則であって、Go には要らないはずだった。Go のクロージャはレキシカルに環境を捕らえるので、`Local` な join を入れ子クロージャの中へインライン展開しても、そのクロージャが必要な変数を捕らえるだけである。

そこで「実際に値として*格納*される位置」だけを escape とする変種（`invoke` の consumer、`apply`/`construct` の consumer リスト、`project` の ret）を実装し、スコープ境界で分類をやり直さず定義ごとに 1 つの map を通す形に変えた。

**dispatch 数も実行時間も完全に同一**（Level 1: 9,028,448 / 0.28s、fib-deep: 18,815,850 / 変化なし）。生成ソースの差は 11,829,129 → 11,828,684 バイト、445 バイトだけだった。

理論的には正しく、より多くの join を `Local` にするが、そうなる join が実プログラムのホットパスにほぼ存在しない。`Escape.lean` が 90 行増えるだけなので revert した。

### 4.11 ClosureConv 相当を Go でもやる — 残りは 0%

Zig の `ClosureConv` のうち Go に持ち込む価値があるのはどこか、3 つに分けて測った。

**(a) `classifyJoins`** — 移植済みで、本調査で最大の効果が出た部分（Level 1 0.38 → 0.30s、dispatch 11.28M → 9.03M）。

**(b) ラムダリフティングと明示的 captures** — Go では**損**になる。Go のクロージャは funcval 1 個で済むが、`Closure{Code, Captures []Value}` はオブジェクトとスライスで 2 個になる。

| 3 変数を捕らえるクロージャ 1 個 | allocs/op | ns |
|---|---|---|
| Go のクロージャ（現行） | 1 | 12.7 〜 13.8 |
| ClosureConv 形式 | 2 | 24.8（**1.8〜1.95 倍**） |

**(c) ANF が可能にする `Peephole`（scrutinee タプル除去）** — **0%。**

生成 Go ソースを直接書き換えて融合を実装した（`m_X_v := mkStruct(tupleTag, A, B)` とその直後の単一アーム select を、`f_0 := A; f_1 := B` に潰す。Level 1 で 265 箇所、fib-deep で 142 箇所）。

| | 値コンストラクタ | Go の malloc | 確保バイト数 | 時間 |
|---|---|---|---|---|
| Level 1 現在 | 1,945,517 | 11,107,241 | 445 MB | 271.1 ms ± 2.8 |
| Level 1 融合後 | 1,358,801 | **7,234,443**（-35%） | **301 MB**（-32%） | **272.0 ms ± 2.2** |
| fib-deep 現在 | 3,399,078 | 13,960,582 | — | 0.29s |
| fib-deep 融合後 | 1,578,193 | **13,960,579**（変化なし） | — | 0.29s |

hyperfine で 40 回ずつ測って **271.1ms 対 272.0ms、差は 0.3% 以内**。malloc を 35%、確保バイト数を 32% 減らして、時間は 1ms も動かない。

fib-deep に至っては **Go の malloc 数がまったく変わらなかった**。`mkStruct` が inline され、その場で分解されるだけのタプルは escape しないので、**Go のコンパイラが既にスタックに置いていた**。融合は Go の escape 解析が済ませていた仕事をなぞっただけだった。

### 4.12 割り当て回数はボトルネックではない

4.4（`mkStruct` 固定アリティ、-4.4M）、4.7（整数タグ、オブジェクト縮小）、4.11(c)（タプル融合、-3.9M / -32% バイト）の 3 つが独立に同じ結論を出した。**Go のこのワークロードでは、小さなオブジェクトの確保は限界費用がほぼゼロである。**

3.3 の微小ベンチが示した「クロージャ 1 個 12.7ns」は、確保だけを回すループの値であって実プログラムの限界費用ではない。この数字に基づく「Level 1 の 54% は割り当て」という見積もりは誤りだった。

Level 1 の 271ms を dispatch 数で割ると 1 dispatch あたり 30ns。うち dispatch 自体は 3.63ns（12%）、割り当ては実質 0%。**残りの約 88% は生成コードそのものの仕事**——func 値経由の間接呼び出し、interface の型アサーション、ポインタ越しのフィールド読み、そして自己ホスト評価器自身のアルゴリズム（環境探索、パターン照合）である。最後のものはバックエンドに依存しない。
...
**実測した。** Zig 0.16 で、handler の prototype を `fn (self: Value, a0: Value, a1: Value) Value` に統一し、引数をレジスタ渡しの固定パラメータにした最小例を書いて確認した。

- **Debug でも ReleaseFast でもコンパイルが通り、2000 万ステップでスタックが伸びない。**
- **関数ポインタ経由でも通る。** 継続の識別子は静的に決まらないので、これは必須条件だった。
- 速度は 3000 万ステップの ReleaseFast で **トランポリン 4.67 ns/step に対し musttail 0.67 ns/step、7 倍**。

つまり `docs/zig-backend.md` の却下理由は「現在の設計では正しい」が、Deegen の言うとおり設計側を変えれば解消する。

Zig バックエンドへの影響見積もり:

| | dispatch 数 | 削減見込み（4ns/dispatch） |
|---|---|---|
| selfhost-l1 | 9.03M | 0.036s（0.24s の 15%） |
| selfhost-l2 | 16.28e9 | 約 65s（245s の 27%） |

実装は Zig バックエンドの呼び出し規約の全面変更になる（`Action`/`run`/`mkAction` の削除、`applyCovalue` など prototype の合わない helper の call site 展開、`forceField` の通常呼び出し化、`Emit.lean` の全終端）。RC の推論は変わらない——`Action` が「move」だったのと同様、直接の末尾呼び出しも move である。別 PR とするのが妥当。
...
## 6.4 Chez は同じだけ割り当てている

「Go の遅さは構造的」と結論する前に、**Chez が同じ量の仕事をしているか**を確かめる必要があった。生成 `.scm` を `(statistics)` で挟んで累積割り当て量を測る。

| | Chez が割り当てたバイト数 | Go |
|---|---|---|
| fib-deep | 442,875,744 | 約 560 MB |
| selfhost Level 1 | **803,192,816** | 445,393,904 |

**Level 1 では Chez が Go の 1.8 倍割り当てていながら、実行は 2.8 倍速い。** つまり Chez のコンパイラが仕事を減らしているのではなく、1 回あたりの単価が違う。

Level 1 を 1 reduction あたりに割ると、Go は 0.26s / 9.03M = 28.8ns、Chez は 0.08s / 約 9M = 8.9ns。Go の内訳は dispatch 3.6ns（12%）、割り当て 1.23 回 × 12.7ns = 15.6ns（54%）、残りの実処理 約 9.6ns（33%）。Chez の同じ 3 つはおおよそ 1ns / 5.4ns / 2.5ns である。

**3 つすべてで 3〜4 倍差がついており、単一のレバーでは埋まらない。** Go の割り当ては nursery の bump ではなくサイズクラス経由、dispatch は末尾呼び出しではなくトランポリン、実処理は interface 越しのボックス値の追跡である。

## 6.5 Chez との比較を分解する

「Chez に匹敵する性能か」は、そのままでは測れない。Chez は **毎回の起動でスクリプトをコンパイルする**ので、`scheme --script` の実測にはコンパイル時間が含まれる。生成された `.scm` から最後の main 呼び出し行だけを削ると、その分を切り出せる。

| | Chez: 読み込み + コンパイル | Chez: 全体 | Chez: 実行のみ | Go: 全体 |
|---|---|---|---|---|
| fib-deep | 0.15s | 0.19s | **0.04s** | 0.29s |
| selfhost Level 1 | 0.60s | 0.70s | **0.10s** | 0.28s |
| 例題（`examples/malgo/*`、10本） | 0.15〜0.18s | 同左 | ほぼ 0 | 0.01s |

読み方は 2 つに分かれる。

**生成コードの実行速度では、Chez が Go を 2.7〜7 倍上回る。** これは構造的で、埋められない。Go のトランポリンだけで fib-deep に 18.8M × 3.63ns = **0.068s** かかり、これは Chez が fib-deep 全体の実行に使う 0.04s を既に超えている。Chez は proper tail call を持つのでトランポリンを 1 度も踏まない。Go には `musttail` が無く（§5）、この 0.068s は Go の ABI が決める下限である。

**1 回の起動から終了までで見ると、Go が Level 1 で 2.5 倍、短いスクリプトで 15 倍速く、fib-deep でだけ Chez が 1.5 倍速い。** Chez のコンパイル時間は生成コードのサイズに比例するので、Malgo の Prelude を読む程度のプログラムでも 0.15s、自己ホストコンパイラでは 0.60s かかる。単一バイナリの Go はこれを一切払わない。

nix-config のタスクスクリプト（Go バックエンドの消費者候補、短命なプログラムを頻繁に起動する）にとっては後者が効く指標であり、そこでは Go が大差で優位に立つ。長時間回る純粋計算では Chez が速い。
...
| `args []Value` を固定引数へ | Level 1 0.30 → 0.29s、fib-deep 0.32 → 0.30s | **採用**（3〜6%） |
| `Struct.Tag` を整数へ | 0.30 → 0.30s | 見送り |
| `GOGC` の引き上げ | 固定引数化の後は差が消える | 見送り |
| escape 判定のレキシカル化 | dispatch 数・時間とも同一 | 見送り（`Escape.lean` が 90 行増えるだけ） |
| musttail dispatch | Go に手段が無い | 実現不可 |

### コーパス全体での比較

2 点のマイクロベンチではなく、golden コーパス 86 ケースすべてを端から端まで通した合計壁時計（3 周のうち最良）。
...
**Go が 16.3 倍速い。** Chez は 1 ケースごとにスクリプトをコンパイルし直すので、短いプログラムを多数実行する形では差が開く。これが実際の利用形態に最も近い測り方である。

### 最終的な位置

| | Go（開始時） | Go（現在） | Zig | Chez（全体） | Chez（実行のみ） |
|---|---|---|---|---|---|
| selfhost Level 1 | 0.41s | **0.27s** | 0.24s | 0.69s | 0.08s |
| fib-deep | 0.48s | **0.29s** | 0.31s | 0.19s | 0.04s |
| golden 86 ケース合計 | — | **0.76s** | — | 12.39s | — |

Go は fib-deep で Zig を上回り、Level 1 で Zig との差を 1.7 倍から 1.13 倍まで詰めた。

Chez に対しては、**実際に実行する形（コーパス全体を端から端まで）では 16.3 倍速い**。生成コードの実行速度そのものでは 3 倍前後届かないが、その差は「Go に末尾呼び出しが無いこと」だけでは説明できない——割り当て・dispatch・実処理のすべてで 3〜4 倍の単価差がある（§6.4）。
